...
import { IconChevronDown, IconEraser } from "@tabler/icons-react";
...
function ToDoCard(props: { todo: ApiToDo; onUpdate: () => void }) {
  ...
  function handleDelete() {
    ...
    deleteToDo(props.todo._id).then((_res) => {
      props.onUpdate()
    }).catch(console.error)
  }
    ...
    <Paper shadow="sm" p="md" radius="md" >
      <Grid align="center">
        <Grid.Col span="content">
          <Checkbox checked={done} onChange={handleDone} styles={{input: {cursor:"pointer"}}} />
        </Grid.Col>
        <Grid.Col span="auto">
          <Text fz="lg" strikethrough={done}>{props.todo.title}</Text>
        </Grid.Col>
        <Grid.Col span="content">
          <Group spacing="xs">

          <ActionIcon size="sm" color="blue" variant="filled" onClick={toggle}>
            <IconChevronDown style={{ transform: `scaleY(${opened ? -1 : 1})`, transition: "transform 0.1s ease-out"}} />
          </ActionIcon>
          {done  && <ActionIcon size="sm" color="red" variant="filled" onClick={handleDelete}>
            <IconEraser />
          </ActionIcon>}
          </Group>
        </Grid.Col>
      ...
      <Collapse in={opened}>
        ...
        <Text size="sm" color="dimmed" strikethrough={done}>
